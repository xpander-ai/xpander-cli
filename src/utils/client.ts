...
export function createClient() {
  ...
  if (!apiKey) {
    throw new Error(
      'API key not found. Please run "xpander configure" to set up your credentials.',
    );
  }
...
/**
 * Validate an API key against the Xpander API
 */
export async function validateApiKey(apiKey: string): Promise<boolean> {
  try {
    ...
    // Don't output errors during validation
    return false;
  }
}
