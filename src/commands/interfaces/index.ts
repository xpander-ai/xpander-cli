...
export function configureInterfacesCommands(program: Command): void {
  const interfacesCmd = program
    ...
    .option(
      '-s, --simple',
      'Launch simple list view instead of enhanced explorer',
    )
    .action(async (options) => {
      if (options.interactive) {
        await interactiveInterfacesMode();
        return;
      } else if (options.simple) {
        // If simple mode is requested, show help to list commands
        interfacesCmd.help();
        return;
      }

      // Default action - use enhanced explorer
